...
import { Animated, StyleSheet, useWindowDimensions, ViewStyle, View } from 'react-native';

export interface ExpadingDotProps {
  data: Array<Object>;
...
export function DotIdicator({ data, scrollX, containerStyle, dotStyle, inActiveDotColor, inActiveDotOpacity, expandingDotWidth, activeDotColor }: ExpadingDotProps) {
  ...
  return (
    <View style={[styles.containerStyle, containerStyle]}>
      {data.map((_, index) => {
        ...
        return <Animated.View key={`dot-${index}`} style={[ styles.dotStyle, dotStyle, { width: expand }, { opacity }, { backgroundColor: colour } ]}/>
      })}
    </View>
  );
}
